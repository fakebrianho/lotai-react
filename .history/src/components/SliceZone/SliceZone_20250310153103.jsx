...
const processedSlices = slices.reduce((acc, currentSlice) => {
	if (currentSlice.slice_type === 'slider_image') {
		// Find or create the slider group
		const lastItem = acc[acc.length - 1]
		if (lastItem && lastItem.slice_type === 'slider_group') {
			// Add to existing group
			lastItem.images.push(currentSlice.primary)
		} else {
			// Create new group
			acc.push({
				slice_type: 'slider_group',
				id: `slider-group-${acc.length}`,
				images: [currentSlice.primary],
			})
		}
	} else {
