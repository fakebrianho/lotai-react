...
import VideoGallery from '@/app/vp/components/VideoGallery/VideoGallery'
import VideoInfo from '@/app/vp/components/VideoInfo/VideoInfo'
import VideoEmbed from '@/app/vp/components/VideoEmbed/VideoEmbed'
import VideoSlider from '@/app/vp/components/VideoSlider/VideoSlider'
import Collab from '@/app/vp/components/Collab/Collab'
import Collaborators from '@/app/vp/components/Collaborators/Collaborators'
	...
	return (
		<>
			{slices.map((slice, idx) => {
				if (slice.slice_type === 'slider_image') {
				...
				} else if (slice.slice_type === 'video_image') {
					videoImages.push(slice)
					if (
						idx === slices.length - 1 ||
						slices[idx + 1].slice_type !== 'video_image'
					) {
						const videoGallery = (
							<VideoGallery key={`slideshow-${idx}`}>
								{videoImages.map((sliderSlice, i) => (
									<VideoSlider
										key={`slider-image-${i}`}
										data={sliderSlice}
									/>
					...
					return null
				} else if (slice.slice_type === 'collaborator') {
					collabArray.push(slice)
					if (
						idx === slices.length - 1 ||
						slices[idx + 1].slice_type !== 'collaborator'
					) {
						const collabContainer = (
							<Collaborators key={`slideshow-${idx}`}>
								{collabArray.map((sliderSlice, i) => (
									<Collab
										key={`slider-image-${i}`}
										data={sliderSlice}
									/>
